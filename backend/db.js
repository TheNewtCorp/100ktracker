  ...
  return new Promise((resolve, reject) => {
    ...
    db.all('PRAGMA table_info(user_invoices)', (err, columns) => {
      ...
      const newColumns = [
        ...
        { name: 'payment_processor', type: "TEXT DEFAULT 'square'" },
        { name: 'payment_id', type: 'TEXT' },
        { name: 'payment_status', type: 'TEXT' },
        { name: 'customer_name', type: 'TEXT' },
        { name: 'customer_email', type: 'TEXT' },
        { name: 'customer_phone', type: 'TEXT' },
        { name: 'customer_company', type: 'TEXT' },
        { name: 'overdue_at', type: 'DATETIME' },
      ...
      if (columnsToAdd.length === 0) {
        ...
        updateInvoiceConstraints(resolve, reject);
        return;
      }
...
function updateInvoiceConstraints(resolve, reject) {
  console.log('Updating invoice table constraints...');

  // Check if we need to update constraints by looking at the current schema
  db.get("SELECT sql FROM sqlite_master WHERE type='table' AND name='user_invoices'", (err, row) => {
    if (err) {
    ...
    console.log('Current user_invoices schema sample:', currentSchema.substring(0, 200) + '...');
