    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className='flex items-center justify-between'>
        <button
          ...
          className='flex items-center gap-2 px-4 py-2 bg-charcoal-slate border border-champagne-gold/20 rounded-lg text-platinum-silver hover:border-champagne-gold/50 transition-all duration-200'
        >
          <Filter size={16} className='text-champagne-gold/60' />
          <span className='text-sm'>Filters {activeFiltersCount > 0 && `(${activeFiltersCount})`}</span>
          {filterState.isExpanded ? (
            <ChevronUp size={16} className='text-champagne-gold/60' />
          ) : (
            <ChevronDown size={16} className='text-champagne-gold/60' />
          )}
        </button>
      ...
      {filterState.isExpanded && (
        <div className='space-y-4 p-4 bg-obsidian-black/30 rounded-lg border border-champagne-gold/10'>
          {/* Primary Filters Row */}
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4'>
            ...
            <div className='relative'>
              <select
                ...
                className='w-full px-3 py-2 text-sm bg-charcoal-slate border border-champagne-gold/20 rounded-lg text-platinum-silver focus:border-champagne-gold/50 focus:ring-2 focus:ring-champagne-gold/20 transition-all duration-200'
              >
